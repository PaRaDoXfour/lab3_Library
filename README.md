# 📚 Library Management System - Developer Guide

Цей проєкт призначений для управління бібліотечним фондом, реєстрації видачі та повернення книг.

---

## 🛠 Підготовка середовища

Якщо ви щойно встановили ОС, виконайте наступні кроки:

### 1. Встановлення необхідного ПЗ
Вам знадобляться:
* **Java Development Kit (JDK) 17 або вище:** Завантажити з Oracle або через пакетний менеджер (напр., `brew install openjdk@17` для macOS).
* **Apache Maven:** Для збирання проєкт та керування залежностями.
* **Git:** Для клонування репозиторію.

### 2. Клонування проєкту
```bash
git clone [https://github.com/PaRaDoXfour/Lab4_Library.git](https://github.com/PaRaDoXfour/Lab4_Library.git)
cd Lab4_Library
```

### 3. Налаштування залежностей
Проєкт використовує Maven. Для завантаження всіх бібліотек виконайте:
```bash
mvn clean install
```
---
## ⚙️ Конфігурація та запуск

### Створення "Бази даних" (Файлу даних)
Проєкт використовує файл `input.txt` як сховище. Переконайтеся, що файл знаходиться в кореневій директорії проєкту. Структура файлу повинна відповідати формату:

```plaintext
[Library]
Назва бібліотеки
Адреса

[PaperBook]
Назва
Автор
Рік
ISBN
...
```

### Запуск проєкту у режимі розробки
Для запуску головного класу Driver через Maven:

```bash
mvn exec:java -Dexec.mainClass="org.example.Driver"
```
---
## ⌨️ Базові команди

* **Збирання проєкту:** `mvn compile`
* **Запуск тестів:** `mvn test`
* **Перевірка стилю коду (Checkstyle):** `./check_all.sh` (для Linux/macOS) або `mvn checkstyle:check`
* **Очищення тимчасових файлів:** `mvn clean`
---
## 📝 Основні операції для розробника

* **Додавання нової книги:** Через меню в консолі (дані автоматично збережуться в `input.txt` при виході).
* **Валідація:** При зміні логіки перевіряйте класи `Exception`, що відповідають за валідацію ISBN та дат.
* **Stream API:** Для зміни алгоритмів пошуку або сортування редагуйте відповідні методи в класі `Library`.
---
## ⚡ Автоматизація

У проєкті доступні скрипти для швидкої роботи:

* **`./run_dev.sh`** — швидкий запуск для розробника (Maven).
* **`./deploy.sh`** — повний цикл збірки та запуску для Production.
* **`Procfile`** — конфігурація для менеджерів процесів.
* **`./backup.sh`** — створення резервної копії даних та логів з перевіркою цілісності.
---
## 📝 Стандарти документування коду

Всі учасники проєкту мають дотримуватися стандарту **Javadoc** для забезпечення прозорості архітектури.

### Що підлягає обов'язковому документуванню:
1. **Класи та інтерфейси:** Обов'язковий короткий опис призначення класу.
2. **Публічні методи:** - Опис того, що робить метод.
    - Опис кожного параметра через `@param`.
    - Опис значення, що повертається через `@return`.
    - Опис помилок через `@throws`.
3. **Складні алгоритми:** Якщо всередині методу є нетривіальна логіка, вона має бути пояснена inline-коментарями (`//`).

### Приклад оформлення:
```java
/**
 * Розраховує термін оренди книги.
 * @param days Кількість днів.
 * @return Дата повернення.
 */
public LocalDate calculateDueDate(int days) { ... }
```
